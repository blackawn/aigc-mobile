...
<template>
  <div class="flex h-full flex-col p-4">
    <div class="mb-2 flex items-center">
      <Search v-model="searchValue" placeholder="请输入搜索关键词" class="flex-1"
        :class="(!mutualStore.novelContentSelected ? '!py-0 !pl-0 !pr-2' : '!p-0')" autocomplete="off"
        @click-left-icon="getNovelHistoryListData(searchValue)" @search="getNovelHistoryListData(searchValue)">
        <template #left-icon>
          <Icon icon="tabler:search" class="text-xl" />
        </template>
      </Search>
      <Button v-show="!mutualStore.novelContentSelected" size="small" type="primary" class="!px-3.5"
        @click="handleCreateClick">
        <Icon icon="fluent:add-12-filled" class="text-sm" />
      </Button>
    </div>
    <div class="scroll-hidden flex-1 overflow-x-hidden">
      <div v-if="mutual.load" class="flex h-full items-center justify-center">
        <Loading />
      </div>
      <div v-else-if="isEmptyList" class="flex h-full items-center justify-center">
        <Empty description="没有数据" />
      </div>
      <div v-for="item in formatNovelHistoryList" :key="item.label">
        <Divider v-if="!isRealEmpty(item.list)" class="!my-2 text-sm" content-position="left">
          {{ item.label }}
        </Divider>
        <div class="flex flex-col gap-y-1.5">
          <div v-for="(record, index) in item.list" :key="record.novel_id" class="flex rounded bg-neutral-100 px-4 py-3"
            :class="{
              '!bg-primary text-white': (Number(route.params.id) === record.novel_id)
            }" :data-index="index">
            <div class="flex flex-1 items-center truncate active:text-neutral-400" @click="handleRecordClick(record)">
              <div class="mr-2 ">
            ...
            <div class="flex gap-x-2">
              <div class="p-1 active:text-neutral-400" @click.stop="handleToggleTopClick(record)">
                <Icon v-if="record.is_top === 1" icon="lucide:arrow-down-to-line" />
                <Icon v-else icon="lucide:arrow-up-to-line" />
              </div>
              <div class="p-1 active:text-neutral-400" @click.stop="handleEditClick(record)">
                <Icon icon="lucide:edit" />
              </div>
              <div class="p-1 active:text-neutral-400" @click.stop="handleDeleteClick(record)">
                <Icon icon="mi:delete" />
              </div>
            </div>
          </div>
